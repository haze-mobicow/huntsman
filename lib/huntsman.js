...
module.exports = function( options ){
  ...
  huntsman.proxy = huntsman.options.proxy || new (require('./proxy/http'))(huntsman.options);
  ...
  huntsman.next = function( isSeed ){
    ...
    var stats = huntsman.proxy.getStats();
    if( 'string' === typeof( uri ) ){
      if( true === isSeed || huntsman.match( uri ) ){
        huntsman.proxy.request( uri, function( err, res ){
          huntsman.emit( 'HUNTSMAN_RESPONSE', err, res );
          huntsman.emit( 'request', uri );
        });
      }
      else console.log( '[skip] uri cannot be matched to callback', uri );
    } else if (stats.waiting == 0 && huntsman.queue.count() === 0 ){
      huntsman.stop();
    }
  };
  ...
  huntsman.pause = function( waitms ){
    clearInterval( huntsman.loop );
    huntsman.emit( 'HUNTSMAN_PAUSE' );
  };
  ...
  huntsman.proxyStats = function(){
    var stats = huntsman.proxy.getStats();
    huntsman.emit( 'HUNTSMAN_PROXY_STATS', stats );
    return stats;
  };
  return huntsman;
};
